...
> **Status: Historical; superseded by repository separation.** This document
> records the pre-rebaseline design in which Writer and the Reviewer suite
> shared one distribution. The active boundary is documented in the standalone
> local `rka-reviewers` repository; do not use this file as current product
> architecture.
